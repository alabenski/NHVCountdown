...
BACK_BTN.addEventListener("click", ()=>{ SETTINGS_VIEW.style.display="none"; MAIN_VIEW.style.display="block"; });
SAVE_BTN.addEventListener("click", ()=>{
  ...
  SETTINGS_VIEW.style.display="none"; MAIN_VIEW.style.display="block";
});
...
function newConfettiPiece(){ return {x:Math.random()*window.innerWidth, y:-Math.random()*window.innerHeight, w:Math.random()*10+6, h:Math.random()*6+4, angle:Math.random()*Math.PI*2, speedX:(Math.random()-0.5)*2, speedY:Math.random()*3+2, color:`hsl(${Math.floor(Math.random()*360)},85%,55%)`, rotationSpeed:(Math.random()-0.5)*0.2}; }
function startConfetti(){ confettiActive=true; confettiEndTime=Date.now()+60_000; confettiPieces=Array.from({length:200},newConfettiPiece); if(!rafId) drawConfetti(); }
function drawConfetti(){ CTX.clearRect(0,0,CANVAS.width,CANVAS.height); for(let p of confettiPieces){ CTX.save(); CTX.translate(p.x,p.y); CTX.rotate(p.angle); CTX.fillStyle=p.color; CTX.fillRect(-p.w/2,-p.h/2,p.w,p.h); CTX.restore(); p.x+=p.speedX; p.y+=p.speedY; p.angle+=p.rotationSpeed; if(p.y>window.innerHeight+20){ Object.assign(p,newConfettiPiece(),{y:-10}); } } if(Date.now()<confettiEndTime){ rafId=requestAnimationFrame(drawConfetti); } else{ confettiActive=false; confettiPieces=[]; CTX.clearRect(0,0,CANVAS.width,CANVAS.height); if(rafId){cancelAnimationFrame(rafId); rafId=null;} } }
...
updateOnce(); setInterval(updateOnce,1000);
